...
function DragDropBoard(){
  ...
  const { pushState, goBack, goForward, canGoBack, canGoForward } = useHistory()
  ...
  showNotification(
    'Задача обновлена!',
    {
      body: `Задача "${updatedTasks.find(t => t.id === taskId).title}" была обновлена`,
      tag: 'task-updated'
    }
  );
...
const handleFinishEditing = () => {
  ...
  const handleDrop = (taskId, newColumnId) => {
    ...
    console.log(`Задача "${task.title}" переехала из ${task.columnId} to ${newColumnId}`)
  }
  ...
  const handleDeleteTask = (taskId) => {
    ...
    if (window.confirm(`Вы уверены, что хотите удалить задачу "${taskToDelete.title}"?`)) {
  ...
  useEffect(() => {
    ...
    if (movedTaskId) {
      const taskElement = document.querySelector(`[data-task-id="${movedTaskId}"]`)
      if (taskElement) {
